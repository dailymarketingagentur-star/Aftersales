...
import type { ActionSequence, ActionTemplate } from "@/types/integration";

export default function WebhookSequenzDetailPage() {
  ...
  useEffect(() => {
    ...
    // Alle Templates laden (Webhook + Jira, da gemischte Sequenzen moeglich)
  ...
  async function handleDeleteSequence() {
    ...
    try {
      ...
      router.push("/integrationen/webhooks/sequenzen");
    } catch (err) {
      setMessage(err instanceof Error ? err.message : "Fehler beim Löschen.");
      setIsError(true);
    }
                ...
                >
                  ...
                  {templates.map((tpl) => (
                    <option key={tpl.id} value={tpl.id}>
                      [{tpl.target_type === "webhook" ? "Webhook" : "Jira"}] {tpl.name} ({tpl.slug})
                    </option>
                  ))}
                </select>
              </div>
